#Here we begin with the path finder working.

#imported some important libraries and modules to work on.
...
class robot:
    ...
    def __init__(self,parentMaze,x=None,y=None,shape='square',goal=None,filled=False,footprints=False,color:COLOR=COLOR.blue):
        '''
        parentmaze-->  The maze on which robot is placed.
        x,y-->  Position of the robot i.e. cell inside which robot will be placed
        ...
        _orient-->  You don't need to pass this
                    It is used with arrow shape robot to shows it turning
        position--> You don't need to pass this
                    This is the cell (x,y)
        _head-->    You don't need to pass this
                    It is actually the robot.
        _body-->    You don't need to pass this
                    Tracks the body of the robot (the previous positions of it)
        '''
        ...
        self._parentMaze._robots.append(self)
        if goal==None:
    ...
    def _RCCW(self):
        '''
        To Rotate the robot in Counter Clock Wise direction
        '''
    ...
    def _RCW(self):
        '''
        To Rotate the robot in Clock Wise direction
        '''
        def pointNew(p,newOrigin):
            return (p[0]-newOrigin[0],p[1]-newOrigin[1])
        w=self._parentMaze._cell_width
...
class maze:
    ...
    def __init__(self,rows=10,cols=10):
        ...
        _robots-->  A list of aganets on the maze
        markedCells-->  Will be used to mark some particular cell during
                        path trace by the robot.
        _
        '''
        self.rows=rows
        self.cols=cols
    ...
    def setupPlayButton(self,mDict):
       self.mDict=mDict
    ...
    def CreateMaze(self,x=1,y=1,pattern=None,loopPercent=0,saveMaze=False,loadMaze=None,theme:COLOR=COLOR.dark):
        ...
        robot(self,*self._goal,shape='square',filled=True,color=COLOR.green)
        if saveMaze:
    ...
    def _drawMaze(self,theme):
        ...
        '''
        def showpath():
            self.tracePath(self.mDict,delay=150,kill=False)
        self._LabWidth=50 # Space from the top for Labels
        self._win=Tk()
        self._win.state('zoomed')
        self._win.title('Path_Finder🌈')
        btn = Button(self._win, text = 'Reach the goal',
                          command = showpath)
        btn.pack(side = 'right')
        messagebox.showinfo("WELCOME","HERE WE WILL SEE HOW DIJKSTRA ALGORITHM WORKS",)
        scr_width=self._win.winfo_screenwidth()
        scr_height=self._win.winfo_screenheight()
        self._win.geometry(f"{200}x{300}+0+0")
    ...
    def _redrawCell(self,x,y,theme):
        '''
        To redraw a cell.
        With Full sized square robot, it can overlap with maze lines
    ...
    def enableArrowKey(self,a):
        '''
        To control an robot a with Arrow Keys
        '''
    ...
    def enableWASD(self,a):
        '''
        To control an robot a with keys W,A,S,D
        '''
    ...
    def _tracePathSingle(self,a,p,kill,showMarked,delay):
        '''
        An interal method to help tracePath method for tracing a path by robot.
        '''
        
        def killAgent(a):
            '''
            if the robot should be killed after it reaches the Goal or completes the path
            '''
